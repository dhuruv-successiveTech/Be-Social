...
const CreatePost = ({ onPostCreated }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(
        "http://localhost:4000/create-post-with-media",
        {
          method: "POST",
          headers: {
      ...
      // Ensure 'id', 'content', 'media', 'createdAt' are present
      const postData = result.data || result;
      const postId = postData.id || postData._id;
      const newPost = {
        id: postId,
        content: postData.content || content,
        media: postData.media || mediaFiles.map((f) => f.url || f.name || ""),
      ...
      try {
        ...
        let feedData;
        try {
          feedData = client.cache.readQuery({
            query: GET_FEED,
        ...
        let userPostsData;
        try {
          userPostsData = client.cache.readQuery({
  ...
  return (
    <div
      className={`bg-gray-800 shadow-md hover:shadow-lg rounded-xl p-6 mb-6 transition-all duration-200`}
    >
      <form onSubmit={handleSubmit}>
        <div className="flex items-start space-x-4">
          ...
          <div className="flex-1">
            <textarea
              className={`w-full p-4 border-0 rounded-xl focus:ring-2 resize-none transition-all duration-200  'bg-gray-700/50 text-gray-200 placeholder-gray-500 focus:ring-indigo-400`}
        ...
        <div
          className={`mt-4 flex items-center justify-between border-t pt-4 border-gray-700`}
        >
          <div className="flex items-center space-x-4">
            <button
              type="button"
              className={`flex items-center space-x-2 transition-colors text-gray-400 hover:text-indigo-400`}
          ...
          <button
            ...
            className={`px-6 py-2.5 bg-indigo-500 text-white rounded-lg hover:bg-indigo-600 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition-all duration-200 focus:ring-offset-gray-800 ${
